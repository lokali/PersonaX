...
class CausalPhenotye(nn.Module):
    def __init__(
        self, 
        n_modalities: int, 
        input_dims: List[int], 
    ...
    def _select_encoder(self, input_type: str, input_dim: int, latent_dim: int, eta_dim: int, hidden_dim: int):
        ...
        if input_type == "m1":
            return m1Encoder(input_dim, latent_dim, eta_dim, hidden_dim)
        elif input_type == "m2":
            return m2Encoder(input_dim, latent_dim, eta_dim, hidden_dim)
        elif input_type == "m3":
            return m3Encoder(input_dim, latent_dim, eta_dim, hidden_dim)
        else:
    ...
    def _select_decoder(self, input_type: str, latent_dim: int, eta_dim: int, output_dim: int, hidden_dim: int):
        ...
        if input_type == "m1":
            return m1Decoder(latent_dim + eta_dim, output_dim, hidden_dim)
        elif input_type == "m2":
            return m2Decoder(latent_dim + eta_dim, output_dim, hidden_dim)
        elif input_type == "m3":
            return m3Decoder(latent_dim + eta_dim, output_dim, hidden_dim)
        else:
            raise ValueError(f"Unsupported input type: {input_type}")
