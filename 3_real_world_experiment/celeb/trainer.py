...
from models.causalmultimodal_1m import CausalMultimodal_1m 
...
def evaluate_model_performance(epoch, model, input_dims, test_data, args, tracking):
    ...
    Z_true = test_data[:, 0:5]
    Z_others = test_data[:, -14:]            # [5-personality, 8-shared]
    Z_pred = z_test.detach().cpu().numpy()  # [3-shared, 10-images, 5-personality]  578*17

    Z_features = test_data[:, -18:]


    if epoch in [100, 300, 500, 700, 999]:
        np.save(f"{args.result_path}/Z_pred_epoch{epoch}.npy", Z_pred)
        np.save(f"{args.result_path}/Z_features_epoch{epoch}.npy", test_data[:, -24:]) 


    r2 = calculate_r2(Z_true, Z_pred[:, -5:])
    mcc = calculate_mcc(Z_true, Z_pred[:, -5:], Z_others, tracking["true_kci_matrix"])
...
def train_model(args, input_dims, train_data, test_data):
    ...
    elif args.case_id == 'athlete_1m':
        model = CausalMultimodal_1m(
            n_modalities = args.n_modalities,
            input_dims = input_dims,
            input_types = args.input_types,
            latent_dims = args.latent_dims,
    ...
    for epoch in range(args.num_epochs):
        ...
        tracking["true_kci_matrix"] = true_kci_matrix
    ...
    if args.ground_truth:
     ...
     # Save the results
    import os 
    path = f"./result/{args.case_id}/results.csv"
    if not os.path.exists(path):
        f = open(path, "w")
        f.write(f"seed,lr,recon_coef,kl_coef,best_mcc,best_r2,epochs\n")
    else:
        f = open(path, "a+")
    f.write(f"{args.seed},{args.lr},{args.recon_coef},{args.kl_coef},{tracking['best_mcc']:.4f},{tracking['best_r2']:.4f},{tracking['best_epoch']}\n")
    f.close()
