...
class mnistEncoder(nn.Module):
    def __init__(
        ...
        self.fc3 = nn.Linear(128, latent_dim + eta_dim)
...
class CausalMultimodal_1m(nn.Module):
    def __init__(
        self, 
        n_modalities: int, 
        input_dims: List[int], 
        ...
        self.num_latent = latent_dims[0] + latent_dims[-1]
    ...
    def forward(self, args, epoch, inputs: List[torch.Tensor]):
        ...
        # z_gap_loss = (torch.norm(z_group[1] - z_group[0], p=2) + torch.norm(z_group[2] - z_group[0], p=2)) / 2
        ...
        # reconstruction_loss += z_gap_loss * 0.001  
