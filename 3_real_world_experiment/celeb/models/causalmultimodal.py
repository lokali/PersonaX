...
class CausalMultimodal(nn.Module):
    def __init__(
        ...
        self.num_latent = latent_dims[0] + latent_dims[-1] + 3
        ...
        self.encoder_for_shared = self._select_encoder('mnist', np.sum(input_dims), 3, 1, None)
        self.decoder_for_shared = self._select_decoder('mnist', 3, 1, np.sum(input_dims), None)
    ...
    def forward(self, args, epoch, inputs: List[torch.Tensor]):
        ...
        ##################################
        # Step 0.1: Encoder & Decoder for the shared latents 
        ##################################
        ...
        z_gap_loss = (torch.norm(z_group[0] - z_group[1], p=2) + torch.norm(z_group[0] - z_group[2], p=2)) / 2 + \
            (torch.norm(z_group[3] - z_group[4], p=2) + torch.norm(z_group[3] - z_group[5], p=2)) / 2 
