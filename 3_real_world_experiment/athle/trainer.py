...
def evaluate_model_performance(epoch, model, input_dims, test_data, args, tracking):
    ...
    Z_true = test_data[:, -13:-8]
    Z_others = test_data[:, -8:]            # [5-personality, 8-shared]
    Z_pred = z_test.detach().cpu().numpy()  # [2-shared, 5-images, 5-personality]  578*12 

    # np.save(f"./result/{args.case_id}/Z_pred.npy", Z_pred)
    if epoch in [100, 500, 700, 1000]:
